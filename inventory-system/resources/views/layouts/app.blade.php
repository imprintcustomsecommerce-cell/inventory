...
<div class="min-h-full lg:flex" x-data="{ open: false }">
    ...
    <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-zinc-900 px-4 py-6 flex flex-col
        ...
        <form action="{{ route('search.index') }}" method="GET" class="mb-4">
            <div class="relative">
                <svg class="pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg>
                <input type="search" name="q" value="{{ request('q') }}" placeholder="Search…"
                       class="w-full rounded-lg border-0 bg-zinc-800 py-2 pl-9 pr-3 text-sm text-zinc-100 placeholder:text-zinc-500 focus:bg-zinc-700 focus:ring-2 focus:ring-brand-400">
            </div>
        </form>

        <!-- Nav -->
        <nav class="flex-1 space-y-1">
            <p class="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-zinc-600">Overview</p>

            @php $u = auth()->user(); @endphp
            @if($u->isMaterialsStaff())
                <a href="{{ route('materials.index') }}" class="nav-link {{ request()->routeIs('materials.*') ? 'nav-link-active' : '' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9"/></svg>
                    <span>Materials</span>
                </a>

                <p class="px-3 pb-2 pt-5 text-xs font-semibold uppercase tracking-wider text-zinc-600">Purchasing</p>
            ...
            @else
                ...
                <a href="{{ route('products.index') }}" class="nav-link {{ request()->routeIs('products.*') ? 'nav-link-active' : '' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m16.5 0a48.667 48.667 0 00-16.5 0m16.5 0l-1.5 1.5m-13.5-1.5l1.5 1.5m0 0v8.25m6-8.25v8.25"/></svg>
                    <span>Products</span>
                </a>

                @if($u->canSeeMaterials())
                    <a href="{{ route('materials.index') }}" class="nav-link {{ request()->routeIs('materials.*') ? 'nav-link-active' : '' }}">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9"/></svg>
                        <span>Materials</span>
                    </a>
                ...
                @unless($u->isAdmin())
                    @php $pendingRequests = \App\Models\StockRequest::visibleTo($u)->where('status', 'pending')->count(); @endphp
                    <a href="{{ route('requests.index') }}" class="nav-link {{ request()->routeIs('requests.*') ? 'nav-link-active' : '' }}">
                        ...
                        @if($pendingRequests > 0)<span class="rounded-full bg-brand-400 px-2 py-0.5 text-xs font-bold text-zinc-900">{{ $pendingRequests }}</span>@endif
                    </a>
                @endunless
                ...
                <p class="px-3 pb-2 pt-5 text-xs font-semibold uppercase tracking-wider text-zinc-600">CRM</p>
                ...
                <a href="{{ route('feedback.index') }}" class="nav-link {{ request()->routeIs('feedback.*') ? 'nav-link-active' : '' }}">
                ...
                <a href="{{ route('calendar.index') }}" class="nav-link {{ request()->routeIs('calendar.*') ? 'nav-link-active' : '' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9-6h.008v.008H12v-.008zM12 15h.008v.008H12V15zm0 2.25h.008v.008H12v-.008zM9.75 15h.008v.008H9.75V15zm0 2.25h.008v.008H9.75v-.008zM7.5 15h.008v.008H7.5V15zm0 2.25h.008v.008H7.5v-.008zm6.75-4.5h.008v.008h-.008v-.008zm0 2.25h.008v.008h-.008V15zm0 2.25h.008v.008h-.008v-.008zm2.25-4.5h.008v.008H16.5v-.008zm0 2.25h.008v.008H16.5V15z"/></svg>
                    <span>Calendar</span>
                </a>

                <a href="{{ route('quality.index') }}" class="nav-link {{ request()->routeIs('quality.*') ? 'nav-link-active' : '' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/></svg>
                    <span>Quality / QC</span>
                </a>

                <a href="{{ route('deliveries.index') }}" class="nav-link {{ request()->routeIs('deliveries.*') ? 'nav-link-active' : '' }}">
                ...
                @if($u->isAdmin())
                    ...
                    <a href="{{ route('promo-codes.index') }}" class="nav-link {{ request()->routeIs('promo-codes.*') ? 'nav-link-active' : '' }}">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z"/><path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z"/></svg>
                        <span>Promo Codes</span>
                    </a>

                    <a href="{{ route('commissions.index') }}" class="nav-link {{ request()->routeIs('commissions.*') ? 'nav-link-active' : '' }}">
